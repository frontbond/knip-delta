...
function groupByFile(issues: FlatIssue[]): Map<string, FlatIssue[]> {
  const groups = new Map<string, FlatIssue[]>();
...
function renderIssueList(issues: FlatIssue[]): string {
...
export function toMarkdown(diff: DiffResult): string {
  ...
  if (diff.totals.added === 0) {
    ...
    out.push(
      `### ⚠️ Knip: ${diff.totals.added} new issue(s) introduced by this PR`,
    );
    out.push("");
  ...
  if (diff.totals.removed > 0) {
    out.push("");
    out.push(
      `<details><summary>✅ ${diff.totals.removed} issue(s) resolved by this PR</summary>\n`,
    );
  ...
  if (diff.totals.unchanged > 0) {
    out.push("");
    out.push(
      `<details><summary>${diff.totals.unchanged} pre-existing issue(s), not introduced by this PR</summary>\n`,
    );
