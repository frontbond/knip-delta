...
export function toText(diff) {
    ...
    if (diff.totals.added === 0) {
        out.push(`No new Knip issues introduced. (${diff.totals.unchanged} pre-existing issue(s) untouched, ${diff.totals.removed} resolved.)`);
    }
    else {
    ...
    if (diff.totals.unchanged > 0) {
        out.push("");
        out.push(`${diff.totals.unchanged} pre-existing issue(s) untouched by this change.`);
    }
    return out.join("\n");
}
