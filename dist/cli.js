...
function readReport(path) {
    ...
    try {
        ...
        throw new Error(`Could not read Knip report at "${path}": ${error.message}`);
    }
    try {
        return JSON.parse(raw);
    }
    catch {
        throw new Error(`"${path}" is not valid JSON. Generate it with: knip --reporter json > ${path}`);
    }
}
function formatOutput(diff, format) {
    switch (format) {
        ...
        case "text":
            return toText(diff);
        default:
            throw new Error(`Unknown --format "${format}". Expected one of: json, markdown, text.`);
    }
}
const program = new Command();
program
    .name("knip-delta")
    .description("Diff two `knip --reporter json` reports and show only what a change introduced or fixed.")
    .version("0.1.0");
program
    .command("diff")
    .description("Compare a base and head Knip JSON report")
    .requiredOption("--base <path>", "Path to the Knip JSON report for the base/target branch")
    .requiredOption("--head <path>", "Path to the Knip JSON report for the head/PR branch")
    .option("--format <format>", "json | markdown | text", "text")
    .option("--out <path>", "Write the report to a file instead of stdout")
    .option("--fail-on <mode>", "When to exit non-zero: 'added' (default) or 'never'", "added")
    .action((opts) => {
